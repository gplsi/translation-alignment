#!/bin/bash
#SBATCH --job-name=amic_paralelo-va-es-sentences               # Job name
#SBATCH --output=logs/amic_paralelo-va-es-sentences-[%j].out   # Standard output (stdout) file
#SBATCH --error=logs/amic_paralelo-va-es-sentences-[%j].err    # Standard error (stderr) file
...
python -m src.filter_items "output/amic-paralelo (plain)/va-es-sentence/" --lang0 va --lang1 es --enable-length --enable-ner --verbose
