...
def process_directory(input_dir, output_suffix, transform_function, verbose=False, keep_empty=False):
    output_dir = f"{input_dir if input_dir[-1] != '/' else input_dir[:-1]}.{output_suffix}"
    ...
    pbar = tqdm(total=total_files, desc="Processing files") if verbose else None
...
if __name__ == "__main__":
    ...
    parser.add_argument("output_suffix", help="Path to the output directory")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    parser.add_argument("--keep-empty", action="store_true", help="Process empty files")
    args = parser.parse_args()

    process_directory(args.input_dir, args.output_suffix, transform_function, verbose=args.verbose, keep_empty=args.keep_empty)
